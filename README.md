...
1. Explain the difference between `RDBMS` and `SQL`.
  - Relational Database Management System (RDBMS) is one of the systems used to manage databases. Structured Query Language (SQL) is the language used to interact the database.
2. Why do tables need a `primary key`?
  - Primary keys are needed to uniquely identify a record in a table.
3. What is the name given to a table column that references the primary key on another table.
  - Foreign Key
4. What do we need in order to have a _many to many_ relationship between two tables.
  - An additional table that holds foreign keys that reference primary keys on those two tables. 
